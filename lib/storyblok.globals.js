...
const PROJECT_CACHE = path.join(PROJECT_ROOT, 'public', 'cache')
...
module.exports = async function fetchCMSGlobals() {
    // const locales = NEXT_PUBLIC_ACTIVE_LANGUAGES ? NEXT_PUBLIC_ACTIVE_LANGUAGES.split(',') : ['en'] // osman changed this on 20250505
    const locales = NEXT_PUBLIC_ACTIVE_LANGUAGES ? NEXT_PUBLIC_ACTIVE_LANGUAGES.split(',') : ['nl']
    try {
        ...
        fs.writeFileSync(path.join(PROJECT_CACHE, 'globals.json'), JSON.stringify(globals, null, 2))
